...
#define ADP5360_BUCK_CFG_SS_MASK        GENMASK(7, 6)
#define ADP5360_BUCK_CFG_SS_POS         6u
#define ADP5360_BUCK_CFG_BST_ILIM_MASK  GENMASK(5, 3)
#define ADP5360_BUCK_CFG_BST_ILIM_POS   3u
#define ADP5360_BUCK_CFG_BUCK_ILIM_MASK GENMASK(5, 4)
#define ADP5360_BUCK_CFG_BUCK_ILIM_POS  4u
#define ADP5360_BUCK_CFG_BUCK_MODE_MASK BIT(3)
#define ADP5360_BUCK_CFG_BUCK_MODE_POS  3u
#define ADP5360_BUCK_CFG_STP_MASK       BIT(2)
#define ADP5360_BUCK_CFG_DISCHG_MASK    BIT(1)
#define ADP5360_BUCK_CFG_EN_MASK        BIT(0)

/* Buck/boost output voltage setting register. */
#define ADP5360_BUCK_OUTPUT_VOUT_MASK GENMASK(5, 0)
#define ADP5360_BUCK_OUTPUT_VOUT_POS  0u
#define ADP5360_BUCK_OUTPUT_DLY_MASK  GENMASK(7, 6)
...
static int regulator_adp5360_set_voltage(const struct device *dev, int32_t min_uv, int32_t max_uv)
{
	...
	return mfd_adp5360_reg_update(config->mfd_dev, config->desc->out_reg,
				      ADP5360_BUCK_OUTPUT_VOUT_MASK, idx);
}
...
static int regulator_adp5360_get_voltage(const struct device *dev, int32_t *volt_uv)
{
	...
	raw_reg = FIELD_GET(ADP5360_BUCK_OUTPUT_VOUT_MASK, raw_reg);
					       ...
					       int32_t max_ua)
{
	const struct regulator_adp5360_config *config = dev->config;
	uint16_t idx;
	uint8_t mask = ADP5360_BUCK_CFG_BUCK_ILIM_MASK;
	int ret;
	...
	if (config->desc->is_buckboost) {
		mask = ADP5360_BUCK_CFG_BST_ILIM_MASK;
	}
...
static int regulator_adp5360_set_mode(const struct device *dev, regulator_mode_t mode)
{
	...
	return mfd_adp5360_reg_update(config->mfd_dev, config->desc->cfg_reg,
				      ADP5360_BUCK_CFG_BUCK_MODE_MASK, mode);
}
...
static int regulator_adp5360_get_mode(const struct device *dev, regulator_mode_t *mode)
{
	...
	*mode = FIELD_GET(ADP5360_BUCK_CFG_BUCK_MODE_MASK, val);
...
static int regulator_adp5360_set_active_discharge(const struct device *dev, bool active_discharge)
{
	...
	return mfd_adp5360_reg_update(config->mfd_dev, config->desc->cfg_reg,
				      ADP5360_BUCK_CFG_DISCHG_MASK, active_discharge);
}
...
static int regulator_adp5360_get_active_discharge(const struct device *dev, bool *active_discharge)
{
	...
	if (FIELD_GET(ADP5360_BUCK_CFG_DISCHG_MASK, raw_reg)) {
...
static int regulator_adp5360_init(const struct device *dev)
{
	...
	msk = ADP5360_BUCK_OUTPUT_DLY_MASK;
	...
	nval = FIELD_PREP(ADP5360_BUCK_CFG_SS_MASK, config->ss_idx) |
	       FIELD_PREP(ADP5360_BUCK_CFG_STP_MASK, config->stp_en);

	if (config->desc->is_buckboost) {
		nval |= FIELD_PREP(ADP5360_BUCK_CFG_BST_ILIM_MASK, config->ilim_idx);
	} else {
		nval |= FIELD_PREP(ADP5360_BUCK_CFG_BUCK_ILIM_MASK, config->ilim_idx);
	}
