...
static int ltc4286_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_ALL:
		...
		command = LTC4286_CMD_READ_TEMP;
		ret = ltc4286_cmd_read(dev, cmd_ptr, 1, val_ptr, 2);
		if (ret) {
			return ret;
		}

		drv_data->temp_sample = value;
			       ...
			       struct sensor_value *val)
{
	...
	switch ((int)chan) {
	case SENSOR_CHAN_VOLTAGE:
		data = drv_data->vout_sample * LTC4286_ADC_VAL_TO_UVAL;
		if (cfg->vrange_set) {
			data = (data * LTC4286_ADC_VIN_VOUT_100V_FACTOR) /
			       LTC4286_ADC_VIN_VOUT_100V_SCALE;
			val->val1 = data / LTC4286_ADC_VAL_TO_UVAL;
			val->val2 = data % LTC4286_ADC_VAL_TO_UVAL;
		} else {
			data = data * LTC4286_ADC_VIN_VOUT_25V_FACTOR /
			       LTC4286_ADC_VIN_VOUT_25V_SCALE;
			val->val1 = data / LTC4286_ADC_VAL_TO_UVAL;
			val->val2 = data % LTC4286_ADC_VAL_TO_UVAL;
		}
		break;
	case SENSOR_CHAN_LTC4286_VIN:
		data = drv_data->vin_sample * LTC4286_ADC_VAL_TO_UVAL;
		if (cfg->vrange_set) {
			data = data * LTC4286_ADC_VIN_VOUT_100V_FACTOR /
			       LTC4286_ADC_VIN_VOUT_100V_SCALE;
			val->val1 = data / LTC4286_ADC_VAL_TO_UVAL;
		...
		break;
	case SENSOR_CHAN_CURRENT:
		data = ((int64_t)drv_data->iout_sample * LTC4286_ADC_VAL_TO_UVAL);
		data = (data * LTC4286_ADC_IOUT_FACTOR) / cfg->r_sense_uohms;
		data = data / LTC4286_ADC_IOUT_SCALE;
		val->val1 = data / LTC4286_ADC_VAL_TO_UVAL;
		val->val2 = data % LTC4286_ADC_VAL_TO_UVAL;
		break;
	case SENSOR_CHAN_POWER:
		data = drv_data->pin_sample * LTC4286_ADC_VAL_TO_UVAL;
		if (cfg->vrange_set) {
			data = (data * LTC4286_ADC_PIN_100V_FACTOR) / cfg->r_sense_uohms;
			data = data / LTC4286_ADC_PIN_100V_SCALE;
			val->val1 = data / LTC4286_ADC_VAL_TO_UVAL;
			val->val2 = data % LTC4286_ADC_VAL_TO_UVAL;
		} else {
			data = (data * LTC4286_ADC_PIN_25V_FACTOR) / cfg->r_sense_uohms;
	...
	case SENSOR_CHAN_AMBIENT_TEMP:
		data = drv_data->temp_sample * LTC4286_ADC_TEMP_SCALE - LTC4286_ADC_TEMP_K_TO_C;
				       ...
				       const struct sensor_value *val)
{
	...
	case SENSOR_CHAN_VOLTAGE:
		if (cfg->vrange_set) {
			...
			code = (uint16_t)(scaled_value / (LTC4286_ADC_VIN_VOUT_100V_FACTOR *
							  LTC4286_ADC_VAL_TO_UVAL));
			return ltc4286_cmd_write(dev, LTC4286_CMD_VOUT_LOWER_THRES_WARN,
						 (uint8_t *)&code, 2);
		} else {
			scaled_value = scaled_value * LTC4286_ADC_VIN_VOUT_25V_SCALE;
			code = (uint16_t)(scaled_value / (LTC4286_ADC_VIN_VOUT_25V_FACTOR *
							  LTC4286_ADC_VAL_TO_UVAL));
			return ltc4286_cmd_write(dev, LTC4286_CMD_VOUT_LOWER_THRES_WARN,
						 (uint8_t *)&code, 2);
		}
	case SENSOR_CHAN_LTC4286_VIN:
		if (cfg->vrange_set) {
			...
			code = (uint16_t)(scaled_value / (LTC4286_ADC_VIN_VOUT_100V_FACTOR *
							  LTC4286_ADC_VAL_TO_UVAL));
			return ltc4286_cmd_write(dev, LTC4286_CMD_VIN_LOWER_THRES_WARN,
						 (uint8_t *)&code, 2);
		} else {
			scaled_value = scaled_value * LTC4286_ADC_VIN_VOUT_25V_SCALE;
			code = (uint16_t)(scaled_value / (LTC4286_ADC_VIN_VOUT_25V_FACTOR *
	...
	case SENSOR_CHAN_AMBIENT_TEMP:
		...
		code = (uint16_t)(scaled_value - LTC4286_ADC_TEMP_K_TO_C) / LTC4286_ADC_VAL_TO_UVAL;
		return ltc4286_cmd_write(dev, LTC4286_CMD_TEMP_LOWER_THRES_WARN,
					 (uint8_t *)&code, 2);
	default:
		return -EINVAL;
	}
				       ...
				       const struct sensor_value *val)
{
	...
	case SENSOR_CHAN_VOLTAGE:
		if (cfg->vrange_set) {
			...
			code = (uint16_t)(scaled_value / (LTC4286_ADC_VIN_VOUT_100V_FACTOR *
							  LTC4286_ADC_VAL_TO_UVAL));
			return ltc4286_cmd_write(dev, LTC4286_CMD_VOUT_UPPER_THRES_WARN,
						 (uint8_t *)&code, 2);
		} else {
			scaled_value = scaled_value * LTC4286_ADC_VIN_VOUT_25V_SCALE;
			code = (uint16_t)(scaled_value / (LTC4286_ADC_VIN_VOUT_25V_FACTOR *
	...
	case SENSOR_CHAN_LTC4286_VIN:
		if (cfg->vrange_set) {
			...
			code = (uint16_t)(scaled_value / (LTC4286_ADC_VIN_VOUT_100V_FACTOR *
							  LTC4286_ADC_VAL_TO_UVAL));
			return ltc4286_cmd_write(dev, LTC4286_CMD_VIN_UPPER_THRES_WARN,
						 (uint8_t *)&code, 2);
		} else {
			scaled_value = scaled_value * LTC4286_ADC_VIN_VOUT_25V_SCALE;
			code = (uint16_t)(scaled_value / (LTC4286_ADC_VIN_VOUT_25V_FACTOR *
	...
	case SENSOR_CHAN_CURRENT:
		...
		code = (uint16_t)(scaled_value /
				  (LTC4286_ADC_IOUT_FACTOR * LTC4286_ADC_VAL_TO_UVAL));
		ret = ltc4286_cmd_write(dev, LTC4286_CMD_IOUT_UPPER_THRES_WARN, (uint8_t *)&code,
					2);
		if (ret) {
			return ret;
		}
	...
	case SENSOR_CHAN_POWER:
		if (cfg->vrange_set) {
			...
			return ltc4286_cmd_write(dev, LTC4286_CMD_PIN_UPPER_THRES_WARN,
						 (uint8_t *)&code, 2);
		} else {
	...
	case SENSOR_CHAN_AMBIENT_TEMP:
		scaled_value =
			(val->val1 * LTC4286_ADC_VAL_TO_UVAL + val->val2) * LTC4286_ADC_TEMP_SCALE;
		code = (uint16_t)(scaled_value - LTC4286_ADC_TEMP_K_TO_C) / LTC4286_ADC_VAL_TO_UVAL;
		return ltc4286_cmd_write(dev, LTC4286_CMD_TEMP_UPPER_THRES_WARN,
					 (uint8_t *)&code, 2);
	default:
		return -EINVAL;
	}
...
static int ltc4286_cfg_fault_response(const struct device *dev)
{
	...
	val = FIELD_PREP(LTC4286_FAULT_RESPONSE_MASK,
			 (cfg->overcurr_config.no_ignore ? LTC4286_FAULT_RESP_TYP3 : 0)) |
	      FIELD_PREP(LTC4286_FAULT_RETRY_MASK, cfg->overcurr_config.retries);
	...
	/* Overtemperature */
	val = FIELD_PREP(LTC4286_FAULT_RESPONSE_MASK,
			 (cfg->overtemp_config.no_ignore ? LTC4286_FAULT_RESP_TYP2 : 0)) |
	      FIELD_PREP(LTC4286_FAULT_RETRY_MASK, cfg->overtemp_config.retries);
	...
	/* Vin Overvoltage */
	val = FIELD_PREP(LTC4286_FAULT_RESPONSE_MASK,
			 (cfg->overvolt_config.no_ignore ? LTC4286_FAULT_RESP_TYP2 : 0)) |
	      FIELD_PREP(LTC4286_FAULT_RETRY_MASK, cfg->overvolt_config.retries);
	...
	/* Vin Undervoltage */
	val = FIELD_PREP(LTC4286_FAULT_RESPONSE_MASK,
			 (cfg->undervolt_config.no_ignore ? LTC4286_FAULT_RESP_TYP2 : 0)) |
	      FIELD_PREP(LTC4286_FAULT_RETRY_MASK, cfg->undervolt_config.retries);
	...
	/* FET Faults */
	val = FIELD_PREP(LTC4286_FAULT_RESPONSE_MASK,
			 (cfg->fet_config.no_ignore ? LTC4286_FAULT_RESP_TYP1 : 0)) |
	      FIELD_PREP(LTC4286_FAULT_RETRY_MASK, cfg->fet_config.retries);
	...
	op_val = FIELD_PREP(GENMASK(15, 5), cfg->op_tmr) |
		 FIELD_PREP(GENMASK(4, 3),
			    (cfg->overpower_config.no_ignore ? LTC4286_FAULT_RESP_TYP1 : 0)) |
		 FIELD_PREP(GENMASK(2, 0), cfg->overpower_config.retries);
...
static int ltc4286_init(const struct device *dev)
{
	...
	if (!device_is_ready(cfg->i2c_dev.bus)) {
	...
	data_ptr[0] = LTC4286_CMD_ALERT_MASK_VOUT;
	data_ptr[1] = 0;
	...
	data_ptr[0] = LTC4286_CMD_ALERT_MASK_IOUT;
	data_ptr[1] = 0;
	ret = ltc4286_cmd_write(dev, LTC4286_CMD_ALERT_MASK, data_ptr, 2);
	if (ret) {
		return ret;
	...
	data_ptr[0] = LTC4286_CMD_ALERT_MASK_INPUT;
	data_ptr[1] = 0;
	ret = ltc4286_cmd_write(dev, LTC4286_CMD_ALERT_MASK, data_ptr, 2);
	if (ret) {
		return ret;
	...
	data_ptr[0] = LTC4286_CMD_ALERT_MASK_TEMP;
	data_ptr[1] = 0;
	ret = ltc4286_cmd_write(dev, LTC4286_CMD_ALERT_MASK, data_ptr, 2);
	if (ret) {
		return ret;
...
#define LTC4286_DEFINE(inst)                                                                       \
	...
	static const struct ltc4286_dev_config ltc4286_config_##inst = {                           \
		...
		.overcurr_config =                                                                 \
			{                                                                          \
				.retries = DT_INST_PROP(inst, overcurrent_fault_retries),          \
				.no_ignore =                                                       \
					DT_INST_PROP(inst, overcurrent_fault_response_shutdown),   \
			},                                                                         \
		.overvolt_config =                                                                 \
			{                                                                          \
				.retries = DT_INST_PROP(inst, overvolt_fault_retries),             \
				.no_ignore = DT_INST_PROP(inst, overvolt_fault_response_shutdown), \
			},                                                                         \
		.undervolt_config =                                                                \
			{                                                                          \
				.retries = DT_INST_PROP(inst, undervolt_fault_retries),            \
				.no_ignore =                                                       \
					DT_INST_PROP(inst, undervolt_fault_response_shutdown),     \
			},                                                                         \
		.overtemp_config =                                                                 \
			{                                                                          \
				.retries = DT_INST_PROP(inst, overtemp_fault_retries),             \
				.no_ignore = DT_INST_PROP(inst, overtemp_fault_response_shutdown), \
			},                                                                         \
		.overpower_config =                                                                \
			{                                                                          \
				.retries = DT_INST_PROP(inst, overpower_fault_retries),            \
				.no_ignore =                                                       \
					DT_INST_PROP(inst, overpower_fault_response_shutdown),     \
			},                                                                         \
		.fet_config =                                                                      \
			{                                                                          \
				.retries = DT_INST_PROP(inst, fet_fault_retries),                  \
				.no_ignore = DT_INST_PROP(inst, fet_fault_response_bad),           \
			},                                                                         \
		.reboot_gpio = GPIO_DT_SPEC_GET_OR(inst, reboot_gpios, {0}),                       \
		.overpower_gpio = GPIO_DT_SPEC_GET_OR(inst, op_status_gpios, {0}),                 \
		.overcurrent_gpio = GPIO_DT_SPEC_GET_OR(inst, oc_status_gpios, {0}),               \
		.comp_out_gpio = GPIO_DT_SPEC_GET_OR(inst, comp_out_gpios, {0}),                   \
		.fault_out_gpio = GPIO_DT_SPEC_GET_OR(inst, fault_out_gpios, {0}),                 \
		.power_good_gpio = GPIO_DT_SPEC_GET_OR(inst, power_good_gpios, {0}),               \
