...
/********************************************************************************
 * Power Management Subsystem API						*
 ********************************************************************************/
 ...
 static int mfd_adp5360_pm_control(const struct device *dev, enum pm_device_action action)
{
	...
	LOG_INF("PM action %d for ADP5360", action);
	switch (action) {
...
static int mfd_adp5360_init(const struct device *dev)
{
	...
	for (val = 0; val < config->int_sources_size; val++) {
		ret = mfd_adp5360_enable_interrupt_mask(dev, config->int_sources[val]);
		if (ret < 0) {
			LOG_ERR("Failed to configure interrupt mask for bit %u",
				config->int_sources[val]);
			return ret;
		}
	}
...
#define MFD_ADP5360_DEFINE(inst)                                                                   \
	static const uint8_t int_srcs##inst[] = DT_INST_PROP_OR(inst, interrupt_sources, {});      \
                                                                                                   \
	static const struct mfd_adp5360_config mfd_adp5360_config_##inst = {                       \
		...
		.int_sources = int_srcs##inst,                                                     \
		.int_sources_size = DT_INST_PROP_LEN_OR(inst, interrupt_sources, 0),               \
