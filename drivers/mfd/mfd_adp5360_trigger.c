...
int mfd_adp5360_enable_interrupt_mask(const struct device *dev, enum adp5360_interrupt_type type)
{
	...
	if (type >= ADP5360_INTERRUPT_BUCKBSTPGOOD) {
		mask = BIT(type - ADP5360_MFD_INTERRUPT_MASK_PG2_SHIFT);
		return mfd_adp5360_reg_update(dev, ADP5360_MFD_REG_INTERRUPT_ENABLE_2, mask, 1);
	} else {
		mask = BIT(type);
		return mfd_adp5360_reg_update(dev, ADP5360_MFD_REG_INTERRUPT_ENABLE_1, mask, 1);
	}
}
...
/********************************************************************************
 * PGOOD1 and PGOOD2 Interrupt API						*
 ********************************************************************************/
